...
def vol_calc(cur, t):
    ...
    if not conect_test(url):
        return conect_test(url)
...
def animate(i):
    ...
    for i, ax in enumerate(axs):
        ...
        avga[i].append(mean(oferts_list[2 * i][-avg_section:]))
        avgb[i].append(mean(oferts_list[2 * i + 1][-avg_section:]))

        rsi[i].append(calc_rsi(rsi_section, dec[i], inc[i], oferts_list[2 * i + 1]))
        axs[1].set_ylabel("value")
        axs[i].plot(czas[-5:], avga[i][-5:], label='avg asks')
        axs[i].plot(czas[-5:], avgb[i][-5:], label='avg bids')
        ...
        if choice == 'vol':
            ...
            pointers[i].legend(loc='center right')
